...
bool gB_Late = false;
float gF_Tickrate = 0.0128; // 100 tickrate.
...
public Action OnPlayerRunCmd(int client, int &buttons)
{
	...
	if(!gB_EnabledPlayers[client] || (buttons & IN_JUMP) > 0)
	{
		return Plugin_Continue;
	}
	...
	if(++gI_GroundTicks[client] == ((gEV_Type == Engine_CSGO)? 3:5) && (gEV_Type != Engine_CSGO || fStamina <= 350.0)) // 350.0 is a sweetspoot for me.
	{
		if(gEV_Type == Engine_CSGO)
		{
		...
		CreateTimer((gEV_Type == Engine_CSGO)? (gF_Tickrate * 5):gF_Tickrate, Timer_ApplyNewStamina, pack, TIMER_FLAG_NO_MAPCHANGE);

		#if defined DEBUG
		if(gB_Shavit)
		{
			Shavit_PrintToChat(client, "Applied new %s.", (gEV_Type == Engine_CSGO)? "stamina":"friction");
		}
		#endif
	}
