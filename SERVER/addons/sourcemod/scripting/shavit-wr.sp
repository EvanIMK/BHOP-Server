...
bool gB_MySQL = false;
bool hasBonus;
char has1[32] = "{default} | {orange}Bonus";
char has2[16] = "";
int tier;
...
public void OnPluginStart()
{
	...
	RegConsoleCmd("sm_record", Command_WorldRecord, "View the leaderboard of a map. Usage: sm_wr [map]");
	RegConsoleCmd("sm_mrank", Command_WorldRecord, "View the leaderboard of a map. Usage: sm_wr [map]");
	RegConsoleCmd("sm_mtop", Command_WorldRecord, "View the leaderboard of a map. Usage: sm_wr [map]");
	RegConsoleCmd("sm_maptop", Command_WorldRecord, "View the leaderboard of a map. Usage: sm_wr [map]");
	RegConsoleCmd("sm_brank", Command_WorldRecord_Bonus, "View the leaderboard of a map. Usage: sm_bwr [map]");
	RegConsoleCmd("sm_btop", Command_WorldRecord_Bonus, "View the leaderboard of a map. Usage: sm_bwr [map]");
	RegConsoleCmd("sm_bonustop", Command_WorldRecord_Bonus, "View the leaderboard of a map. Usage: sm_bwr [map]");
	...
	RegAdminCmd("sm_dr", Command_Delete, ADMFLAG_RCON, "Opens a record deletion menu interface.");
...
public void Shavit_OnFinish(int client, int style, float time, int jumps, int strafes, float sync, int track, float oldtime, float perfs)
{
	...
	if(iOverwrite > 0)
	{
		...
		{
			FormatEx(sMessage, 255, "%s[%s]%s %T",
				gS_ChatStrings.sVariable, sTrack, gS_ChatStrings.sText, "NotFirstCompletion", LANG_SERVER, gS_ChatStrings.sVariable2, client, gS_ChatStrings.sText, gS_ChatStrings.sStyle, gS_StyleStrings[style].sStyleName, gS_ChatStrings.sText, 
				gS_ChatStrings.sVariable2, sTime, gS_ChatStrings.sText, gS_ChatStrings.sVariable, iRank, gS_ChatStrings.sText, jumps, strafes, sSync, gS_ChatStrings.sText, gS_ChatStrings.sGreen, sDifference);
	...
	if(bIncrementCompletions)
	{
		...
		{
			FormatEx(sMessage, 255, "%s[%s]%s %T",
				gS_ChatStrings.sVariable, sTrack, gS_ChatStrings.sText, "WorseTime", client, gS_ChatStrings.sStyle, gS_StyleStrings[style].sStyleName, gS_ChatStrings.sText, gS_ChatStrings.sVariable2, sTime, gS_ChatStrings.sText, jumps, strafes, sSync, gS_ChatStrings.sWarning, sDifference);
		}
	}
...
{
	char sQuery[128];
	FormatEx(sQuery, 128, "SELECT AVG(time) FROM playertimes WHERE map = '%s' AND style = 0 AND track = 0;", gS_Map);
	gH_SQL.Query(AverageTime_Callback, sQuery, 0, DBPrio_Low);
}

public void AverageTime_Callback(Database db, DBResultSet results, const char[] error, any data)
{   
	if(results == null)
	{
		LogError("Get average time failed. Reason: %s", error);
	...
	{
        float fAvg = results.FetchFloat(0);
        FormatSeconds(fAvg, sAvgMap, 32);
        FormatSeconds(gF_WRTime[0][0], sTimeMap, 32);
    }
}
...
{
	char sQuery[128];
	FormatEx(sQuery, 128, "SELECT AVG(time) FROM playertimes WHERE map = '%s' AND style = 0 AND track = 1;", gS_Map);
	gH_SQL.Query(AverageBonus_Callback, sQuery, 0, DBPrio_Low);
}

void HasBonus()
{
	char sQuery[128];
	FormatEx(sQuery, 128, "SELECT map FROM mapzones WHERE map = '%s' AND track = 1", gS_Map);
	gH_SQL.Query(HasBonus_Callback, sQuery, 0, DBPrio_Low);
}

public void AverageBonus_Callback(Database db, DBResultSet results, const char[] error, any data)
{   
	if(results == null)
	{
		LogError("Get average bonus time failed. Reason: %s", error);
	...
	{
        float fAvg = results.FetchFloat(0);
        FormatSeconds(fAvg, sAvgBonus, 32);
        FormatSeconds(gF_WRTime[0][1], sTimeBonus, 32);  
    }
}

public void HasBonus_Callback(Database db, DBResultSet results, const char[] error, any data)
{   
	if(results == null)
	{
		LogError("Get has bonus failed. Reason: %s", error);

		return;
	}
	
	int count = results.FetchRow();
	{
        if(count > 0)
        {
            hasBonus = true;
...
public void Event_OnPlayerSpawn(Event event, const char[] name, bool dontBroadcast)
{
	...
	CreateTimer(5.0, Timer_PrintDetails, GetClientUserId(client), TIMER_FLAG_NO_MAPCHANGE);
}

public Action Timer_PrintDetails(Handle timer, any data)
{
	...
	if(!StrEqual(sTimeMap, "0.000"))
    {
        CPrintToChat(client, "{blue}MAP {default}Record: {grey2}%s ", sTimeMap);
        CPrintToChat(client, "{blue}MAP {default}Average: {grey2}%s ", sAvgMap); 
    }
	
	if(!StrEqual(sAvgBonus, "0.000"))
    {
        CPrintToChat(client, "{orange}BONUS {default}Record: {grey2}%s ", sTimeBonus);
        CPrintToChat(client, "{orange}BONUS {default}Average: {grey2}%s ", sAvgBonus);
    }
	
	CPrintToChat(client, "{blue}Map{default}: %s | {bluegrey}Tier{default}: %d %s", gS_Map, tier, (hasBonus == true)? has1 : has2);
