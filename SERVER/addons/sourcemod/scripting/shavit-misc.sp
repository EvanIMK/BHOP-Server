...
void UpdateScoreboard(int client)
{
	...
	{
		int rank = Shavit_GetRank(client);
		if(rank == 0)
		{
			rank = 9999;
		}
	...
	if(gB_Rankings)
	{
		SetEntProp(client, Prop_Data, "m_iDeaths", iScore);
	}
}
...
public void Shavit_OnWorldRecord(int client, int style, float time, int jumps, int strafes, float sync, int track)
{
	...
	if(gA_StyleSettings[style].bAnnounce)
	{
...
{
	if(StrContains(gS_StyleStrings[gI_Style[client]].sSpecialString, "segments") != -1)
		return true;
	else if(StrContains(gS_StyleStrings[gI_Style[client]].sSpecialString, "tagteam") != -1)
		return true;
	else 
		return false;
}
