...
public int Native_GetStyleStrings(Handle handler, int numParams)
{
	...
	switch(type)
	{
		...
		case sCategory: return SetNativeString(3, gS_StyleStrings[style].sCategory, size);
	}
...
void StartTimer(int client, int track)
{
		...
		(fSpeed[2] == 0.0 &&(gA_StyleSettings[gA_Timers[client].iStyle].iPrespeed == 2 || SquareRoot(Pow(fSpeed[0], 2.0)+ Pow(fSpeed[1], 2.0))<= 290.0)))
	{
		...
		if(result == Plugin_Continue)
		{
			...
			SetEntProp( client, Prop_Send, "m_iFOV", view_as<int>(gA_StyleSettings[gA_Timers[client].iStyle].iFov));
			SetEntProp( client, Prop_Send, "m_iDefaultFOV", view_as<int>(gA_StyleSettings[gA_Timers[client].iStyle].iFov));
		}
...
bool LoadStyles()
{
	...
	do
	{
		...
		kv.GetString("category", gS_StyleStrings[i].sCategory, sizeof(stylestrings_t::sCategory), "");
		...
		gA_StyleSettings[i].bAnnounce = view_as<bool>(kv.GetNum("announce", true));
		gA_StyleSettings[i].iFov = view_as<bool>(kv.GetNum("fov", 90));
...
bool LoadMessages()
{
	...
	kv.GetString("style", gS_ChatStrings.sStyle, sizeof(chatstrings_t::sStyle), "\x07276f5c");
	kv.GetString("green", gS_ChatStrings.sGreen, sizeof(chatstrings_t::sGreen), "\x07db88c2");
