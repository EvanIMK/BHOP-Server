...
public OnPluginStart()
{
	...
	decl String:PopularPath[PLATFORM_MAX_PATH];
	
	BuildPath(Path_SM, mapListPath, PLATFORM_MAX_PATH, "configs/mapchooser_extended/maps/%s.txt", g_GameModName);
	BuildPath(Path_SM, PopularPath, PLATFORM_MAX_PATH, "configs/mapchooser_extended/maps/popular.txt");
	SetMapListCompatBind("official", mapListPath);
	SetMapListCompatBind("official", PopularPath);
...
public Action:Command_SetNextmap(client, args)
{
	if (args < 1)
	{
		CReplyToCommand(client, "[{blue}MAP{default}] Usage: sm_setnextmap <map>");
	...
	if (!IsMapValid(map))
	{
		CReplyToCommand(client, "[{blue}MAP{default}] %t", "Map was not found", map);
		return Plugin_Handled;
	}
...
{
	ShowActivity2(client, "[{blue}MAP{default}] ", "%t", "Initiated Vote Map");
...
InitiateVote(MapChange:when, Handle:inputlist=INVALID_HANDLE)
{
	...
	if (IsVoteInProgress() || (g_NativeVotes && NativeVotes_IsVoteInProgress()))
	{
		...
		CPrintToChatAll("[{blue}MAP{default}] %t", "Cannot Start Vote", FAILURE_TIMER_LENGTH);
		new Handle:data;
...
//	Call_Finish();
	...
	CPrintToChatAll("[{blue}MAP{default}] %t", "Nextmap Voting Started");
}
						   ...
						   const item_info[][2])
{
	...
	if (strcmp(map, VOTE_EXTEND, false) == 0)
	{
		...
		CPrintToChatAll("[{blue}MAP{default}] %t", "Current Map Extended", RoundToFloor(float(item_info[0][VOTEINFO_ITEM_VOTES])/float(num_votes)*100), num_votes);
	...
	else if (strcmp(map, VOTE_DONTCHANGE, false) == 0)
	{
		...
		CPrintToChatAll("[{blue}MAP{default}] %t", "Current Map Stays", RoundToFloor(float(item_info[0][VOTEINFO_ITEM_VOTES])/float(num_votes)*100), num_votes);
	...
	else
	{
		...
		CPrintToChatAll("[{blue}MAP{default}] %t", "Nextmap Voting Finished", mapName, RoundToFloor(float(item_info[0][VOTEINFO_ITEM_VOTES])/float(num_votes)*100), num_votes);
						   ...
						   const item_info[][2])
{
	...
	if (GetConVarBool(g_Cvar_RunOff) && num_items > 1 && g_RunoffCount < GetConVarInt(g_Cvar_MaxRunOffs))
	{
		...
		if (highest_votes == item_info[1][VOTEINFO_ITEM_VOTES])
		{
			...
			CPrintToChatAll("[{blue}MAP{default}] %t", "Tie Vote", GetArraySize(mapList));
		...
		else if (highest_votes < required_votes)
		{
			...
			CPrintToChatAll("[{blue}MAP{default}] %t", "Revote Is Needed", required_percent);
			SetupWarningTimer(WarningType_Revote, MapChange:g_ChangeTime, mapList);
			return;
		}
	}
