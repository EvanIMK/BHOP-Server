...
AttemptRTV(client)
{
	...
	{
		CReplyToCommand(client, "[{blue}RTV{default}] %t", "RTV Not Allowed");
		return;
	}
		
	if (!CanMapChooserStartVote())
	{
		CReplyToCommand(client, "[{blue}RTV{default}] %t", "RTV Started");
		return;
	}
	
	if (GetClientCount(true) < GetConVarInt(g_Cvar_MinPlayers))
	{
		CReplyToCommand(client, "[{blue}RTV{default}] %t", "Minimal Players Not Met");
		return;			
	}
	
	if (g_Voted[client])
	{
		CReplyToCommand(client, "[{blue}RTV{default}] %t", "Already Voted", g_Votes, g_VotesNeeded);
		return;
	}	
	...
	CPrintToChatAll("[{blue}RTV{default}] %t", "RTV Requested", name, g_Votes, g_VotesNeeded);
	...
	if (EndOfMapVoteEnabled() && HasEndOfMapVoteFinished())
	{
		...
		if (GetNextMap(map, sizeof(map)))
		{
			if (GetConVarBool(g_Cvar_DisplayName))
			{
				...
				CPrintToChatAll("[{blue}RTV{default}] %t", "Changing Maps", mapName);
			}
			else
			{
				CPrintToChatAll("[{blue}RTV{default}] %t", "Changing Maps", map);
			}
